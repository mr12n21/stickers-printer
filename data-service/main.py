...
def determine_prefix(text, config):
    """Determine the prefix based on the configuration from the YAML file."""
    for rule in config.get("prefixes", []):
        pattern = rule.get("pattern")
        prefix = rule.get("prefix")
        if re.search(pattern, text):
            return prefix
    return config.get("default_prefix", "N")  # Default prefix if no rule matches
...
def create_combined_label(variable_symbol, from_date, to_date, guests, prefix, year, output_path):
    ...
    # Convert date format
    to_date_without_year = f"{to_date.split('.')[0]}.{to_date.split('.')[1]}."
    draw.text((10, 50), f"{to_date_without_year}", fill="black", font=font_large)

    # Draw prefix and guests
    draw.text((400, 50), f"{prefix}", fill="black", font=font_large)
...
def process_pdfs(pdf_paths, config_path, output_dir):
    ...
    for pdf_path in pdf_paths:
        ...
        try:
            ...
            # Determine prefix dynamically based on config
            prefix = determine_prefix(text, config)
