...
def find_prefix_and_percentage(text, config):
    ...
    for rule in config.get("prefixes", []):
        ...
        if matches:
            ...
            print(f"Prefix '{label}' matched {len(matches)} times!")
...
def create_combined_label(variable_symbol, from_date, to_date, prefixes, year, output_path, final_output):
    ...
    draw.text((320, 30), f"E", fill="black", font=font_large)
    ...
    if final_output:
        draw.text((10,120), f"{final_output}", fill="black", font=font_large)
